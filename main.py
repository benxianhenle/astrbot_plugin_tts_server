...
class TTSServerPlugin(Star):
    ...
    async def _do_tts(
        ...
        Returns:
            TTS请求结果
        """
        logger.info(f"[TTS Plugin] _do_tts 开始: text={text[:30]}..., voice={voice}, role={role}, reference={reference}")
        ...
        if voice is not None and voice != "":
            ...
            try:
                if " | " in voice:
                    ...
                    logger.info(f"[TTS Plugin] 从voice参数解析: role={role}, reference={reference}")
                else:
                    # 格式不正确，清空role和reference
                    logger.warning(f"[TTS Plugin] voice格式不正确: {voice}")
                    role = ""
                    reference = ""
            except Exception as e:
                logger.error(f"[TTS Plugin] 解析voice字段失败: {e}")
                role = ""
                reference = ""
        else:
            # 如果没有提供voice，使用默认配置中的voice字段
            default_voice = self.cfg.default_params.voice
            logger.info(f"[TTS Plugin] 使用默认voice配置: {default_voice}")
            if default_voice:
                try:
                    if " | " in default_voice:
                        ...
                        logger.info(f"[TTS Plugin] 从默认配置解析: role={role}, reference={reference}")
                    else:
                        # 格式不正确，清空role和reference
                        logger.warning(f"[TTS Plugin] 默认voice格式不正确: {default_voice}")
                        if not role:
                            role = ""
                        if not reference:
                            reference = ""
                except Exception as e:
                    logger.error(f"[TTS Plugin] 解析默认voice字段失败: {e}")
                    if not role:
                        role = ""
                    if not reference:
                        reference = ""
        ...
        logger.info(f"[TTS Plugin] 最终参数: role={role}, reference={reference}, language={language}")
        
        # 验证角色是否存在
        if not role:
            logger.error("[TTS Plugin] 角色未设置，请在配置中选择默认音色，或使用情绪配置设置音色")
            return TTSRequestResult(ok=False, error="角色未设置，请在配置中选择默认音色", text=text)
    ...
    async def on_decorating_result(self, event: AstrMessageEvent):
        """消息装饰器 - 自动将文本转为语音"""
        logger.info(f"[TTS Plugin] ========== 装饰器被触发 ==========")
        
        if not self.cfg.enabled:
            logger.info("[TTS Plugin] 插件未启用，跳过")
            return

        cfg = self.cfg.auto
        logger.info(f"[TTS Plugin] 装饰器检查: enabled={self.cfg.enabled}, only_llm_result={cfg.only_llm_result}, tts_prob={cfg.tts_prob}, max_msg_len={cfg.max_msg_len}")
        
        result = event.get_result()
        if not result:
            logger.info("[TTS Plugin] 无结果，跳过")
            return

        chain = result.chain
        if not chain:
            logger.info("[TTS Plugin] 无消息链，跳过")
            return

        logger.info(f"[TTS Plugin] 消息链段数: {len(chain)}")

        # 只处理LLM结果
        if cfg.only_llm_result and not result.is_llm_result():
            logger.info(f"[TTS Plugin] 非LLM结果被跳过 (only_llm_result={cfg.only_llm_result}, is_llm_result={result.is_llm_result()})")
            return

        # 按概率触发
        rand_val = random.random()
        if rand_val > cfg.tts_prob:
            logger.info(f"[TTS Plugin] 概率触发跳过 (random={rand_val:.3f} > tts_prob={cfg.tts_prob})")
            return
        else:
            logger.info(f"[TTS Plugin] 概率触发通过 (random={rand_val:.3f} <= tts_prob={cfg.tts_prob})")
        ...
        logger.info(f"[TTS Plugin] 消息链分析: 总段数={len(chain)}, Plain段数={len(plain_texts)}")

        # 仅允许只含有Plain的消息链通过
        if len(plain_texts) != len(chain):
            logger.info(f"[TTS Plugin] 消息链包含非Plain内容，跳过 (plain_texts={len(plain_texts)}, chain={len(chain)})")
            return

        # 合并所有Plain文本
        combined_text = "\n".join(plain_texts)
        logger.info(f"[TTS Plugin] 合并文本: 长度={len(combined_text)}, 内容: {combined_text[:50]}{'...' if len(combined_text) > 50 else ''}")

        # 仅允许一定长度以下的文本通过
        if len(combined_text) > cfg.max_msg_len:
            logger.info(f"[TTS Plugin] 文本长度超过限制: {len(combined_text)} > {cfg.max_msg_len}")
            return
        else:
            logger.info(f"[TTS Plugin] 文本长度检查通过: {len(combined_text)} <= {cfg.max_msg_len}")

        # 获取情绪参数
        emotion_params = self._get_emotion_params(combined_text)
        logger.info(f"[TTS Plugin] 情绪参数: {emotion_params}")
        
        # 执行TTS
        logger.info("[TTS Plugin] 开始执行TTS...")
        ...
        if not bool(res):
            ...
            logger.info(f"[TTS Plugin] TTS成功: 音频大小={len(res.data) if res.data else 0} bytes")

        # 替换消息链为语音
        logger.info("[TTS Plugin] 替换消息链为语音")
        chain.clear()
        chain.append(self._to_record(res))
        logger.info("[TTS Plugin] 语音转换完成")
